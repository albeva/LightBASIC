...
bool SymbolTable::exists(const string_view& name, bool recursive) {
    if (m_symbols.find(name) != m_symbols.end()) return true;

    return recursive && m_parent != nullptr && m_parent->exists(name, recursive);
}

Symbol *SymbolTable::find(const string_view& id, bool recursive) {
