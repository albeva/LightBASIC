...
void Emitter::emitExecutable()
{
    ...
    //    
    //    // the output
    //    auto output = m_ctx.output();
    //    string llc_cmd  = "/usr/local/bin/llc -filetype=obj ";
    //    string lld_cmd  = "/usr/local/bin/llvm-ld ";
    ...
    //    ld_cmd += "-lSystem -lcrt1.10.6.o -L\"/usr/lib/\" -macosx_version_min 10.6.0 ";
    //    if (m_ctx.arch() == Architecture::X86_32) {
    //        ld_cmd += "-arch x86_32 ";
    //    } else if (m_ctx.arch() == Architecture::X86_32) {
    //        ld_cmd += "-arch x86_64 ";
    //    }
    //    
    ...
    //    cmd = lld_cmd + '"' + boost::algorithm::join(bc_files, "\" \"") + "\" -o \"" + output.string() + '"';
